...
<template>
  <div
    ...
    data-slot="sidebar"
    :class="cn('text-primary-content flex h-full w-(--sidebar-width) flex-col', props.class)"
    v-bind="$attrs"
  >
    <slot />
  </div>
    ...
    <div
      :class="cn(
        ...
        variant === 'floating' || variant === 'inset'
          ...
          : 'group-data-[collapsible=icon]:w-(--sidebar-width-icon) border-base-300 group-data-[side=left]:border-r group-data-[side=right]:border-l',
        props.class,
      )"
      v-bind="$attrs"
    >
